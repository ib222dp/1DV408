...
class LoginModel {
	...
	public function __construct() {
		...
		$this->dao = new DAO();
		//Hämtar användaruppgifter från textfil
	...
	public function tryLoginWithCookie($username, $encryptedPassword) {
		...
		if (array_key_exists($username, $this->savedCredentials)) {
			...
			if ($this->savedCredentials[$username] == base64_decode($encryptedPassword)) {
	...
	public function setServersideCookieExpirationIfAutoLogin() {
		...
		//same as cookie expiration: 30 days (30*24*60*60) (changed to 5 minutes for testing purposes)
